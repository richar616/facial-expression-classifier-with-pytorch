...
def PREDICT():
    file_list = os.listdir('image_to_predict')
    with torch.no_grad():
        ...
        for i in range(len(file_list)):
            ...
            try:
                image = cv2.imread(('./image_to_predict/'+file_list[i]))
            except:
                print('non image available')
            ...
            def see_boxes():
                for(x,y,w,h) in faces: #get boxes values and store in varibles
                    ...
                    ROI_face = image[y:y+h, x:x+w] #extract the face from the image

                    img = T2(ROI_face) #conver the image to pil

                    img = transform(img) #resize, transform to tensor, normalize, etc
                    
                    img = torch.clone((img))#change data type to float32
                    ...
                    if float(acuracy) > 75: #show only the best results
                        cv2.putText(image, tag , (end_corner_x, y+20), cv2.FONT_HERSHEY_SIMPLEX, .5, (255, 0, 0), 2)#draw the text in the image

                    cv2.rectangle(image, (x,y),(end_corner_x, end_corner_y),color,stroke) # draw the rect

            see_boxes()
            print('result_{}.jpg'.format(i))
            cv2.imwrite('./predicted_images/result_{}.jpg'.format(i),image)
